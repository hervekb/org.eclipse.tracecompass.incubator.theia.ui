...
public class HybridDataProvider extends AbstractTmfTraceDataProvider implements ITimeGraphDataProvider<@NonNull ITimeGraphEntryModel>, IOutputStyleProvider {
	
	 
	    public static final String ID = "org.eclipse.tracecompass.incubator.theia.ui.timegraph.hybrid"; //$NON-NLS-1$
	    ...
	    private TheiaStateSystemAnalysisModule fModule;
	    ExampleTimeGraphDataProvider e;
	     ...
	     */
	    public HybridDataProvider(ITmfTrace trace, TheiaStateSystemAnalysisModule module) {
	        super(trace);
	        fModule = module;
	    }
	    ...
	    public static @Nullable ITmfTreeDataProvider<? extends ITmfTreeDataModel> create(ITmfTrace trace) {
	        ...
	        return module != null ? new TheiaTimeGraphDataProvider(trace, module) : null;
	    }
	    ...
	    public TmfModelResponse<TmfTreeModel<@NonNull ITimeGraphEntryModel>> fetchTree(Map<String, Object> fetchParameters, @Nullable IProgressMonitor monitor) {
	        ...
	        for (Integer quark : ss.getQuarks("Theia", "Spans", "*")) { //$NON-NLS-1$ //$NON-NLS-2$
	        	Long id;
	        	
	        	String att_name= ss.getAttributeName(quark);
	        	for (Integer chid_q : ss.getQuarks("Theia", "Spans", att_name,"*")) { 
	        		
	        		id = fIDToDisplayQuark.inverse().computeIfAbsent(chid_q, q -> sfAtomicId.getAndIncrement());
		             entryList.add(new TimeGraphEntryModel(id, -1, ss.getAttributeName(chid_q)+" "+att_name, ss.getStartTime(), endTime));
	        		
	        	}
	        	id = fIDToDisplayQuark.inverse().computeIfAbsent(quark, q -> sfAtomicId.getAndIncrement());
	        	entryList.add(new TimeGraphEntryModel(id, -1, " ", ss.getStartTime(), endTime));
	    ...
	    private static TimeGraphState getStateFromInterval(ITmfStateInterval statusInterval, long currentEndTime) {
	        ...
	        String styleName = "style" + new Random().nextInt(458963) % 3; //$NON-NLS-1$
