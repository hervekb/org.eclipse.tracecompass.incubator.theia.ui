...
public class ThreadPoolFactory implements IDataProviderFactory {
	
	   private static final IDataProviderDescriptor DESCRIPTOR = new DataProviderDescriptor.Builder()
	            .setId(ThreadPoolDataProvider.ID)
	    ...
	    public @Nullable ITmfTreeDataProvider<? extends ITmfTreeDataModel> createProvider(ITmfTrace trace) {
	        ...
	        if (traces.size() == 1) {
	            return ThreadPoolDataProvider.create(trace);
	        }
	        return TmfTreeXYCompositeDataProvider.create(traces, "Event Loop Factory XY Chart", ThreadPoolDataProvider.ID); //$NON-NLS-1$
	    }
