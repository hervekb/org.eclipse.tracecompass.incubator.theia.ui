...
public class TickFrequencyDataProvider extends AbstractTreeDataProvider<TheiaStateSystemAnalysisModule, TmfTreeDataModel> implements ITmfTreeXYDataProvider<TmfTreeDataModel> {
	
	
	    public static final String ID = "org.eclipse.tracecompass.incubator.theia.ui.xy.freq.dataprovider"; //$NON-NLS-1$
	     ...
	     *            The analysis module
	     */
	    public TickFrequencyDataProvider(ITmfTrace trace, TheiaStateSystemAnalysisModule analysisModule) {
	    ...
	    public static @Nullable ITmfTreeDataProvider<? extends ITmfTreeDataModel> create(ITmfTrace trace) {
	        ...
	        return module != null ? new TickFrequencyDataProvider(trace, module) : null;
	    }
	    ...
	    protected TmfTreeModel<TmfTreeDataModel> getTree(ITmfStateSystem ss, Map<String, Object> fetchParameters, @Nullable IProgressMonitor monitor) throws StateSystemDisposedException {
	        ...
	        for (Integer quark : ss.getQuarks("THEIA","Metrics","Tick_frequency","*")) { //$NON-NLS-1$ //$NON-NLS-2$
	            //int statusQuark = ss.optQuarkRelative(quark, "*"); //$NON-NLS-1$
