...
public class TheiaBackendXYDataProviderFactory implements IDataProviderFactory {

    private static final IDataProviderDescriptor DESCRIPTOR = new DataProviderDescriptor.Builder()
            .setId(TheiaBackendXYDataProvider.ID)
    ...
    public @Nullable ITmfTreeDataProvider<? extends ITmfTreeDataModel> createProvider(ITmfTrace trace) {
        ...
        if (traces.size() == 1) {
            return TheiaBackendXYDataProvider.create(trace);
        }
        return TmfTreeXYCompositeDataProvider.create(traces, "Example time graph data provider", TheiaBackendXYDataProvider.ID); //$NON-NLS-1$
    }
