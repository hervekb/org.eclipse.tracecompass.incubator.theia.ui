...
public class TimerFactory implements IDataProviderFactory {
	
	   private static final IDataProviderDescriptor DESCRIPTOR = new DataProviderDescriptor.Builder()
	            .setId(TimerDataProvider.ID)
	    ...
	    public @Nullable ITmfTreeDataProvider<? extends ITmfTreeDataModel> createProvider(ITmfTrace trace) {
	        ...
	        if (traces.size() == 1) {
	            return TimerDataProvider.create(trace);
	        }
	        return TmfTreeXYCompositeDataProvider.create(traces, "Node timers XY graph data provider", TimerDataProvider.ID); //$NON-NLS-1$
	    }
